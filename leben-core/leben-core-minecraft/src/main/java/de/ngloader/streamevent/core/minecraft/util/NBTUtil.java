...
public class NBTUtil {
	...
	public static Entity binaryToEntity(byte[] binary, Location location) {
		...
		net.minecraft.world.entity.Entity entity = EntityType.loadEntityRecursive(nbt, worldServer, (created) -> {
			created.setPosRaw(location.getX(), location.getY(), location.getZ());
			created.setYBodyRot(location.getPitch());
			created.setYHeadRot(location.getYaw());
			return created;
		});

		if (worldServer.addAllEntitiesSafely(entity, SpawnReason.CUSTOM)) {
